...
class InitialConditionsSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        # create the porous medium (the wall)
        ...
        for i, j, k in self.every_pixel():
            ...
            for n in range(N_porus):
                ...
                dx -= self.dim.x*round(dx/self.dim.x)
                dy -= self.dim.y*round(dy/self.dim.y)
                dz -= self.dim.z*round(dz/self.dim.z)

                if dx*dx + dy*dy + dz*dz < R_porus*R_porus:
