...
class InitialConditionsSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        R_porus = 33 # porus diameter = 100 micrometers, while initial T cell radius = 6 micrometers ---> 4 voxels = 6 micrometers
        N_porus = 10
...
class TCellGrowthSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.base_growth_rate = 0.005
        self.growth_rate_multiplier = 1.0
        ...
        for cell in self.cell_list_by_type(self.TCELL, self.ACTIVATEDTCELL):
            ...
            # If the cell can grow, define growth rates for both types of T cells
                ...
                if cell.type == self.ACTIVATEDTCELL:
                    growth_rate = self.base_growth_rate*self.growth_rate_multiplier
                else:
...
class WallContactTrackerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.wall_contact_cells_file.write("#MCS\tTCells_touching_Wall\tActivatedTCells_touching_Wall\tTotal_cells_touching_Wall\n")
        ...
        self.wall_contact_area_file.write("#MCS\tTCell_Wall_contact_area\tActivatedTCell_Wall_contact_area\tTotal_cell_Wall_contact_area\n")
    ...
    def step(self, mcs):
        ...
        self.wall_contact_cells_file.write(f"{mcs}\t{tcell_touching_wall}\t{activated_tcell_touching_wall}\t{total_cells_touching_wall}\n")
        self.wall_contact_cells_file.flush()

        # update contact area with Wall output
        self.wall_contact_area_file.write(f"{mcs}\t{tcell_wall_contact_area}\t{activated_tcell_wall_contact_area}\t{total_cell_wall_contact_area}\n")
        self.wall_contact_area_file.flush()
...
class DivisionStructureTrackerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.initial_zero_divisions_file = open("Initial_zero_divisions_tracker.txt", "w")
        self.initial_zero_divisions_file.write("#MCS\tInitial_cells\tInitial_cells_with_0_divisions\tPercent_initial_cells_with_0_divisions\n")
        self.initial_zero_divisions_file.flush()
    ...
    def step(self, mcs):
        ...
        initial_cells_with_0_divisions = 0

        for cell in all_cells:
            if cell.id in self.initial_cell_ids:
                if cell.dict.get('division_count', 0) == 0:
                    initial_cells_with_0_divisions += 1

        if self.initial_cells_number > 0:
            percent_initial_cells_with_0_divisions = (initial_cells_with_0_divisions/self.initial_cells_number)*100
        else:
            percent_initial_cells_with_0_divisions = 0.0

        self.initial_zero_divisions_file.write(f"{mcs}\t{self.initial_cells_number}\t{initial_cells_with_0_divisions}\t{percent_initial_cells_with_0_divisions}\n")
        self.initial_zero_divisions_file.flush()
    ...
    def finish(self):
        ...
        self.initial_zero_divisions_file.close()
