...
class InitialConditionsSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        # create the flat wall (activating surface)
        ...
        tcells_created = 0
        max_z_tcell_creation = 25

        while tcells_created < N_tcells:

            # choose a random voxel of the lattice
            ...
            # check if the cell voxels are valid
            valid = True
            ...
            # if the voxels are valid, create the T Cell
...
class HeightMonitoringSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        # stop the simulation if z_top is reached
        if touches_z_top:
...
class TCellGrowthSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.r_max_growth = 3.2
        self.volume_threshold = (4/3)*math.pi*self.r_max_growth*self.r_max_growth*self.r_max_growth
        self.pressure_max_growth = 36
    ...
    def step(self, mcs):
        ...
        for cell in self.cell_list_by_type(self.TCELL, self.ACTIVATEDTCELL):
            ...
            if cell.volume >= self.volume_threshold or cell.dict['pressure'] > self.pressure_max_growth:
                continue
        ...
        if mcs % 100 == 0:
            ...
            for cell in self.cell_list_by_type(self.TCELL, self.ACTIVATEDTCELL):
                ...
                if cell.dict['pressure'] >= self.pressure_max_growth:
...
class TCellMitosisSteppable(MitosisSteppableBase):
    ...
    def start(self):
        ...
        # Plot in real time the number of total divisions
...
class VSRatioAndDistanceTrackerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        # Create dictionaries to store the previous COM and the traveled distance for each cell
        self.prev_COM = {}
        self.D = {}
        
        self.initial_cell_ids = set()
        
        # Initialize dictionaries for all existing cells
        for cell in self.cell_list_by_type(self.TCELL, self.ACTIVATEDTCELL):
    ...
    def step(self, mcs):
        ...
        for cell in self.cell_list_by_type(self.TCELL, self.ACTIVATEDTCELL):
            ...
            if cell.id in self.initial_cell_ids: #only for initial cells
                ...
                # Correct the calculation because of the periodic conditions
