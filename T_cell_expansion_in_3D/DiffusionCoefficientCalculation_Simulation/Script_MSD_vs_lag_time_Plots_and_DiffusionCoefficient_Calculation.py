...
# Save lag_time and MSD data

MSD_data = np.column_stack((lag_time, MSD))

np.savetxt(
    "MSD_vs_lag_time_data.txt",
    MSD_data,
    header="# lag_time MSD",
    fmt="%.10f"
)

files.download("MSD_vs_lag_time_data.txt")
...
# LINEAR REGRESSION AND DIFUSSION COEFFICIENT CALCULOUS
...
print("---------------------------")
print(f"Slope = {slope:.10f} ± {slope_std:.10f}")
print(f"Intercept = {intercept:.10f} ± {intercept_std:.10f}")
print(f"Diffusion Coefficient = {D:.10f} ± {D_std:.10f}")
print(f"R^2 = {R2:.10f}")
